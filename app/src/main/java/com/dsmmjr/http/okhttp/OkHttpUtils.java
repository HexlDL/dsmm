...
import com.dsmmjr.utils.ToastUtils;
            ...
            public void run() {
                if (callback != null) {
                    ...
                    if (System.currentTimeMillis() - currentError > 5000) {
                        ToastUtils.showMessage(-1, "服务器开小差去了...");
    ...
    private void sendSuccessResultCallback(final Object object, final ResultCallback callback) {
        mDelivery.post(new Runnable() {
            ...
            public void run() {
                if (callback != null) {
                    try {
                        ResponseInfoEntity responseInfo = mGson.fromJson(object.toString(), ResponseInfoEntity.class);
                        if (responseInfo.getCode() == -1 || responseInfo.getCode() == -2) {//token过期,期限30天, /未登录
                            Intent intent = new Intent(ACTION_TOKEN_EXPIRE);
                            DsmmApp.getAppContext().sendBroadcast(intent);
                        }/* else if (responseInfo.getCode() == -2) {//未登录
                            Intent intent = new Intent(ACTION_TOKEN_EXPIRE);
                            DsmmApp.getAppContext().sendBroadcast(intent);
                        }*/ else if (responseInfo.getCode() == 1 || responseInfo.getCode() == 0) {
                            callback.onResponse(object);
                        }
                        ToastUtils.showMessage(responseInfo.getCode(), responseInfo.getMsg());
                    } catch (com.google.gson.JsonParseException | NullPointerException | IllegalStateException e) {
                        AlertUtil.t(DsmmApp.getAppContext(), e.getLocalizedMessage(), Toast.LENGTH_SHORT);
