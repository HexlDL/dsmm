...
import com.dsmmjr.R;
import com.dsmmjr.entity.GesturePoint;
import com.dsmmjr.utils.AppUtil;
...
public class GestureContentView extends ViewGroup {
    ...
    private void addChild() {
        for (int i = 0; i < 9; i++) {
            ...
            GesturePoint p = new GesturePoint((screenDispaly[0] - blockWidth * 3) / 2 + leftX,
                    (screenDispaly[0] - blockWidth * 3) / 2 + rightX, topY, bottomY, image, i + 1);
            this.list.add(p);
        }
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            int col = i % 3;
            View v = getChildAt(i);
            v.layout((screenDispaly[0] - blockWidth * 3) / 2 + col * blockWidth + blockWidth / baseNum,
                    row * blockWidth + blockWidth / baseNum,
                    (screenDispaly[0] - blockWidth * 3) / 2 + col * blockWidth + blockWidth - blockWidth / baseNum,
